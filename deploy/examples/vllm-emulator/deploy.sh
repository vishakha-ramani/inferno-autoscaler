...
_kubectl apply -f deploy/examples/vllm-emulator/prometheus-operator/prometheus-deploy-all-in-one.yaml

# Create vllm emulated deployment
_kubectl apply -f deploy/examples/vllm-emulator/vllme-setup/vllme-deployment-with-service-and-servicemon.yaml